...
from personal_rag.metadata_filter import normalize_metadata_filters
from personal_rag.query_rewrite import QueryRewriter, build_query_extensions
from personal_rag.retriever import HybridRetriever
from personal_rag.schema import EvalCase, RetrievalEvalCase, RetrievalHit
...
def load_eval_set(path: str | Path) -> list[EvalCase]:
    ...
    if eval_path.suffix.lower() in {".yml", ".yaml"}:
        ...
        if not isinstance(raw_cases, list):
            raise ValueError("Answer eval YAML must be a list or contain a 'queries' list")
        normalized = []
        for item in raw_cases:
            payload = dict(item)
            if "question" not in payload and "query" in payload:
...
class EvalRunner:
    ...
    def run(self, cases: list[EvalCase], *, top_k: int = 10) -> dict[str, Any]:
        ...
        for case in cases:
            ...
            sources = [hit.source_ref for hit in hits]
            rows.append(
                {
                    "id": case.id,
                    ...
                    "recall_at_5": source_recall(case.expected_sources, sources[:5]),
                    "recall_at_10": source_recall(case.expected_sources, sources[:10]),
                    "mrr_at_10": mrr(case.expected_sources, sources[:10]),
                    "ndcg_at_10": ndcg(case.expected_sources, sources[:10]),
    ...
    def run(
        ...
        *,
        ...
        for case in cases:
            ...
            sources = [hit.source_ref for hit in hits]
            expected_ranks = expected_source_ranks(case.expected_sources, sources)
            row = {
                "id": case.id,
                "query": case.query,
            ...
            if case.expected_sources:
                hit_at_1 = (
                    1.0
                    if sources[:1] and sources[0] in case.expected_sources
                    else 0.0
                )
                row.update(
                    {
                        "hit_at_1": hit_at_1,
                        "recall_at_3": source_recall(case.expected_sources, sources[:3]),
                        "recall_at_5": source_recall(case.expected_sources, sources[:5]),
                        "recall_at_k": source_recall(case.expected_sources, sources[:top_k]),
                        "mrr_at_k": mrr(case.expected_sources, sources[:top_k]),
                        "ndcg_at_k": ndcg(case.expected_sources, sources[:top_k]),
                    }
                )
            rows.append(row)
...
def source_recall(expected: list[str], retrieved: list[str]) -> float:
    if not expected:
        return 1.0
...
def expected_source_ranks(expected: list[str], retrieved: list[str]) -> dict[str, int | None]:
    ranks: dict[str, int | None] = {}
    for source in expected:
        ranks[source] = retrieved.index(source) + 1 if source in retrieved else None
    return ranks
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summary = {
        ...
        "source_recall_at_5": avg("source_recall_at_5"),
