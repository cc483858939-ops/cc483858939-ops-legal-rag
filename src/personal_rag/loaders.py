...
from personal_rag.config import SourceItem
from personal_rag.schema import DocumentChunk
from personal_rag.text import chunk_text, normalize_whitespace, stable_id
    ...
    *,
    ...
    for section_index, (section, section_text) in enumerate(section_blocks):
        ...
        for chunk_index, content in enumerate(chunk_texts):
            ...
            source_ref = _section_source_ref(item.source_ref, section)
            chunks.append(
                DocumentChunk(
                    chunk_id=chunk_id,
...
def _section_source_ref(base_source_ref: str, section: str | None) -> str:
    if not section:
        return base_source_ref
    section_number = re.search(
        r"\b([A-Za-z0-9_.-]+)\b",
        section.replace("Section", "").replace("§", ""),
    )
    if section in base_source_ref or (
        section_number and section_number.group(1) in base_source_ref
    ):
        return base_source_ref
    return f"{base_source_ref}, {section}"
