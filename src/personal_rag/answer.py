...
from personal_rag.schema import Answer, RetrievalHit
from personal_rag.text import normalize_whitespace, tokenize

REFUSAL = (
    "I do not have enough retrieved personal knowledge-base evidence to answer that reliably."
)
...
QUESTION_CONCEPTS = {
    "showmaker": {"showmaker", "world", "mid", "lck"},
    "faker": {"faker", "father", "parent"},
    "打火机": {"打火机", "炫神", "最喜欢", "歌"},
}
...
class ExtractiveAnswerer:
    ...
    def answer(self, question: str, hits: list[RetrievalHit], *, max_sources: int = 4) -> Answer:
        supported_hits = self._supported_hits(question, hits)
        if not supported_hits:
            return Answer(question=question, answer=REFUSAL, sources=[], hits=hits, refused=True)

        selected = supported_hits[:max_sources]
        sentences = self._select_sentences(question, selected)
        if not sentences:
            sentences = [normalize_whitespace(selected[0].text)[:500]]
        sources = list(dict.fromkeys(hit.source_ref for hit in selected))
        body = " ".join(sentences)
        source_text = "; ".join(sources)
        return Answer(
            question=question,
            answer=f"{body} Sources: {source_text}.",
            sources=sources,
            hits=hits,
            refused=False,
        )
    ...
    def _supported_hits(self, question: str, hits: list[RetrievalHit]) -> list[RetrievalHit]:
        ...
        if not query_terms:
            return hits
        required_sources = self._source_terms(question)
        ...
        for hit in hits[:5]:
            ...
            if required_sources & support_terms:
    ...
    def _select_sentences(self, question: str, hits: list[RetrievalHit]) -> list[str]:
        query_terms = self._meaningful_terms(question)
        source_terms = self._source_terms(question)
        concept_terms = self._concept_terms(question)
    ...
    def _hit_terms(self, hit: RetrievalHit) -> set[str]:
        return set(tokenize(f"{hit.title} {hit.source_ref} {hit.section or ''} {hit.text}"))

    def _meaningful_terms(self, text: str) -> set[str]:
        return {
            term
            for term in tokenize(text)
            if len(term) > 1 and term not in STOPWORDS and term not in GENERIC_KB_TERMS
        }

    def _source_terms(self, text: str) -> set[str]:
        terms = set()
        terms.update(re.findall(r"\b\d+\b", text.lower()))
        for phrase in re.findall(r"\b[A-Z][a-z]+(?:\s+v\.\s+[A-Z][a-z]+)?\b", text):
            for token in tokenize(phrase):
                if token not in STOPWORDS and token not in GENERIC_KB_TERMS:
                    terms.add(token)
        return terms
