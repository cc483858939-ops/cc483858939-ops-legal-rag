...
from legal_rag.config import Settings, load_settings
from legal_rag.embeddings import FastEmbedEmbedder, HashingEmbedder, SentenceTransformerEmbedder
from legal_rag.fusion import FusionConfig
from legal_rag.ingest import ingest_manifest
from legal_rag.query_rewrite import QueryRewriter, build_query_rewriter
from legal_rag.rerank import build_optional_reranker
from legal_rag.retriever import HybridRetriever
from legal_rag.store import InMemoryLegalStore, QdrantLegalStore
...
def build_store(settings: Settings, *, offline: bool = False):
    ...
    if offline:
        return InMemoryLegalStore(embedder=embedder)
    return QdrantLegalStore(
