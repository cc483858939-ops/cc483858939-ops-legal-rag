...
from legal_rag.metadata_filter import MetadataFilters, qdrant_filter_as_dict
from legal_rag.rerank import Reranker, rerank_hits
...
class HybridRetriever:
    ...
    def retrieve(
        ...
        *,
        ...
        hits = apply_query_authority_boost(query, hits)
        if trace and boost_stage is not None:
            boosted_count = sum(1 for hit in hits if hit.rank_explanation.get("authority_boost"))
            trace.end_stage(
                boost_stage,
                {
                    "hit_count": len(hits),
...
def apply_query_authority_boost(query: str, hits: list[RetrievalHit]) -> list[RetrievalHit]:
    query_terms = set(tokenize(query)) - AUTHORITY_STOPWORDS
    if not query_terms:
        return hits

    boosted: list[RetrievalHit] = []
    for hit in hits:
        authority_terms = set(tokenize(f"{hit.citation} {hit.title} {hit.section or ''}"))
        overlap = query_terms & authority_terms
        authority_boost = 0.0
        if overlap:
            authority_boost = 0.02 * len(overlap)
        ...
        if authority_boost:
            boosted.append(
                hit.model_copy(
                    update={
                        "fusion_score": hit.fusion_score + authority_boost,
                        "rank_explanation": {
                            **hit.rank_explanation,
                            "authority_overlap": sorted(overlap),
